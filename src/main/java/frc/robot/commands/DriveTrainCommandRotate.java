...
public class DriveTrainCommandRotate extends CommandBase {
    ...
    public void initialize() {
        profile = new TrapezoidProfile(
            ...
            new TrapezoidProfile.State(gyroscope.getAngle(), drivetrain.getVelocity())
        );
    ...
    public boolean isFinished() {
        ...
        return Math.abs(gyroscope.getAngle() - targetPosition) < 0.5 && currentTime > profile.totalTime();
    }

    @Override
    public void execute() {
        power = pidController.calculate(gyroscope.getAngle(), targetPosition);
        TrapezoidProfile.State scheduledState = profile.calculate(Timer.getFPGATimestamp() - startTime);

        power =
            0.5 * scheduledState.velocity +
            pidController.calculate(gyroscope.getAngle(), scheduledState.position);
