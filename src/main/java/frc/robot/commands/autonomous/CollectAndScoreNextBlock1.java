...
public class CollectAndScoreNextBlock1 extends SequentialCommandGroup {
    CollectAndScoreNextBlock1(DrivetrainSubsystem drivetrain, IntakeWheels intakeMotor, ElevatorSubsystem elevator, IntakeArm arm) {
    addRequirements(drivetrain, intakeMotor, elevator, arm);
    
    addCommands(
        new ParallelCommandGroup(
            new SequentialCommandGroup(
                new DriveCommandTurn(drivetrain, -0.3).withTimeout(1),
                new DriveCommandMove(drivetrain, -0.3).withTimeout(0.5)
            ),
